...
export const DocumentCard = ({
  ...
  const preview = useMemo(() => getContentPreview(document.content), [document.content]);
  const isRecent =
      ...
      >
        ...
        <div className="flex items-center gap-3 text-xs text-text-muted shrink-0">
          ...
          {document.last_editor && document.last_editor.id !== document.creator?.id && (
            <>
              <span className="text-white/10">•</span>
              <span className="text-text-muted/80 font-medium">Edited by {document.last_editor.display_name}</span>
            </>
          )}
        </div>
      </Link>
  ...
  return (
    <>
          ...
          {!compact && (
            <div className="relative ml-2 shrink-0" ref={menuRef}>
              <Button
                ...
                className="p-1.5 text-text-muted hover:text-text-primary hover:bg-white/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 border border-transparent"
                icon={<MoreHorizontal className="w-4 h-4" />}
              />

              {showMenu && (
                <div className="absolute right-0 top-full mt-1 w-44 bg-surface-elevated border border-white/10 rounded-xl shadow-2xl shadow-black/50 z-50 py-1 overflow-hidden">
                  <Button
                    variant="ghost"
                    onClick={() => {
                      setShowMenu(false);
                    ...
                    className="w-full justify-start gap-3 px-3 py-2 text-xs text-text-secondary hover:text-text-primary hover:bg-white/5 rounded-none transition-colors font-medium border border-transparent"
                  ...
                  <Button
                    ...
                    className="w-full justify-start gap-3 px-3 py-2 text-xs text-danger hover:bg-danger/10 hover:text-danger rounded-none transition-colors font-medium border border-transparent"
        ...
        >
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              ...
              {document.last_editor && document.last_editor.id !== document.creator?.id && (
                <span className="text-[9px] text-text-muted/60 mt-0.5 font-medium pl-0.5">
                  Edited by {document.last_editor.display_name}
                </span>
              )}
            </div>
            {document.attachments && document.attachments.length > 0 && (
      ...
      <ConfirmModal
        ...
        description={`Are you sure you want to permanently delete "${document.title || "Untitled"}"? This action cannot be undone and will delete all attachments.`}
        confirmText="Delete"
        cancelText="Cancel"
        isDanger={true}
      />
